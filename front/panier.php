...
<div class="container py-2">
    ...
    $idUtilisateur = $_SESSION['utilisateurs']['id_user'] ?? 0;
    ...
    if (isset($_POST['valider'])) {
        $sql = 'INSERT INTO ligne_commande(id_produit,id_cmd,prix,quantite,total) VALUES';
        $total = 0;
        $prixProduits = [];
        foreach ($produits as $produit) {
            $idProduit = $produit['id_produit'];
            $qty = $panier[$idProduit];
            $discount = $produit['promo'];
        ...
        $sqlStateCommande = $pdo->prepare('INSERT INTO commande(id_clt,total) VALUES(?,?)');
        $sqlStateCommande->execute([$idUtilisateur, $total]);
        $idCommande = $pdo->lastInsertId();
        $args = [];
        foreach ($prixProduits as $produit) {
            $id = $produit['id_produit'];
            $sql .= "(:id_produit$id,'$idCommande',:prix$id,:quantite$id,:total$id),";
        }
        $sql = substr($sql, 0, -1);
        $sqlState = $pdo->prepare($sql);
        foreach ($prixProduits as $produit) {
            $id = $produit['id'];
            $sqlState->bindParam(':id_produit' . $id, $produit['id_produit']);
            $sqlState->bindParam(':prix' . $id, $produit['prix']);
            $sqlState->bindParam(':quantite' . $id, $produit['quantite']);
        ...
        <div class="row">
            ...
            if (empty($panier)) {
                ...
                <table class="table">
                    ...
                    foreach ($produits as $produit) {
                        $idProduit = $produit['id_produit'];
                        $totalProduit = calculerRemise($produit['prix'], $produit['promo']) * $panier[$idProduit];
                        $total += $totalProduit;
                        ?>
                        <tr>
                            ...
                            <td> - <?= $produit['promo'] ?> %</td>
                            <td><?php echo calculerRemise($produit['prix'], $produit['promo']) ?> <i class="fa fa-solid fa-dollar"></i></td>
